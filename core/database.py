...
def update_missing_yield_list(ip: str, current_yield: int):
    """ثبت yield_per_page هر پرینتر در فایل missing_yield_printers.txt.

    فرمت جدید هر خط:
        <ip> yield_per_page=<value> status=<default|learned>
                    ...
                    for part in parts[1:]:
                        if part.startswith('yield_per_page='):
                            try:
                                ...
                                row_yield = 2000
                    rows[row_ip] = row_yield

        if int(current_yield or 0) == -1:
            rows.pop(ip, None)
        else:
            rows[ip] = int(current_yield or 2000)

        lines = []
        for row_ip in sorted(rows):
            y = int(rows[row_ip] or 2000)
            status = 'default' if y == 2000 else 'learned'
